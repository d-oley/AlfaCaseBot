...
def evaluate_with_llm(text: str, case_id: int, case_context: str, perfect_solution: str,
                      evaluator=evaluate_solution) -> dict[str, Any]:
    started_at = time.perf_counter()
    LOGGER.info(
    ...
    result = evaluator(text, case_context, perfect_solution)
...
async def evaluate(payload: EvaluateRequest, request: Request):
    ...
    case_text_loader = getattr(request.app.state, "case_text_loader", None)
    if case_text_loader is not None:
        try:
            case_text = await case_text_loader(case_id, cookie)
        except (RuntimeError, ValueError, KeyError):
            LOGGER.exception("model_case_text_load_failed case_id=%s", case_id)
            return logged_response(
                error_payload("CASE_TEXT_LOAD_FAILED", "Не удалось загрузить условие для модели"),
                502,
                "evaluate_model_case_load_failed",
            )
        evaluator = partial(evaluator, case_text=case_text)
...
def create_app(evaluator=evaluate_solution, title="AlfaCaseBot LLM Only API", lifespan=None,
               case_text_loader=None):
    application = FastAPI(title=title, version="1.0.0", lifespan=lifespan)
    application.state.evaluator = evaluator
    application.state.case_text_loader = case_text_loader
